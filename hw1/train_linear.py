#!/usr/bin/env python3
# Linear regresssion
# ./train_test_linear.py [train] [model]
...
def main(argv):
    ...
    # Check closed-form solution for linear regression 
    unary_v = np.array([[1] for i in range(len(x))])
    b_x = np.concatenate((unary_v,x),axis=1)
    w_close=np.dot(np.dot(np.linalg.inv(np.dot(b_x.T,b_x)),b_x.T),y_hat)
    
    # Save model
    with open(argv[2], 'w') as output:
        line = ''
        line += str(b)+','+','.join(list(w.astype(str)))
        #line += '\n'+','.join(list(w_close.astype(str)))
        output.write(line)
        
    '''    
    # Prediction on test set
    test_raw = []
    with open(argv[3], 'r') as test_f:
        for line in test_f:
            line = line.replace('NR', '0.0')
            data = line.strip('\n').split(',')
